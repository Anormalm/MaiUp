...
const MAX_LEVEL = 15; // Upstream level-helper constant; avoids unrelated UI dependencies.
import { roundFloat } from './number-helper.js';
export const RANK_S = {
    minAchv: 97.0,
    factor: 0.2,
    title: 'S',
};
export const RANK_SSS_PLUS = {
    minAchv: 100.5,
    factor: 0.224,
    title: 'SSS+',
};
...
export function getRankIndexByAchievement(achievement) {
    return RANK_DEFINITIONS.findIndex((rank) => {
        return achievement >= rank.minAchv;
    });
}
export function getRankByAchievement(achievement) {
...
export function getFinaleRankTitle(achievement) {
    return getRankTitle(achievement).replace('SSS+', 'SSS');
}
/** Returns recommended levels by rank title */
export function calcRecommendedLevels(rating, ranks) {
